...
-- New Thread: https://v3rmillion.net/showthread.php?tid=1202458&pid=8453067#pid8453067
--[[
    Modified by Linen#3485
    - Better Optimization
    - Replaced Corouitine.wrap with task.spawn [ better + it ends itself, reducing lag when executing a lot of times ]
    - Changed 'wait' to 'task.wait'
    - Ui destroys itself when re-executed
    - All events disconnect itself on re-execution
    - Objects like Sliders, TextBox, Dropdown and Toggles return { Value: the current value of the object }, so you can do "print(object.Value)" to get its value 
    - While using :Set, it will also call the callback function, so you dont gotta worry about calling the callback function urself :D
-----------------------------------
]]

-- Binding Key Example: exampleTab:Bind(text, keyname [ Enum.Keycode or string ], callback)
-- Button Example: exampleTab:Button(text, callback)
-- Slider Example: exampleTab:Slider(text, minimumValue [ optinal ], maximumValue [ optinal ], callback [ optinal ], defaultValue [ optinal ])
-- Toggle Example: exampleTab:Toggle(text, value (default value of the toggle [ true or false ] ), callback (function))
-- Textbox Example: exampleTab:Textbox(text, disappear ( when the textbox is clicked, does the value disappear or not ), callback (function))
-- Dropdown Example: exampleTab:Dropdown(text, list: { }, callback)
-- Label Example: exampleTab:Label(text)
...
local uiName = "uiVapeUIMM2OIUYFYHEUFIOIUHGEFHJIJUHEF"
ui.Name = uiName
Events[#Events+1] = ui
...
else ui.Parent = game:GetService("CoreGui") end

ui.Name = "uiVapeUIMM2OIUYFYHEUFIOIUHGEFHJIJUHEF2"
pcall(function()
    local found = ui.Parent:FindFirstChild(uiName)
    for i,v in next, ui.Parent:GetChildren() do
...
local function MakeDraggable(topbarobject, object)
    ...
    Events[#Events+1] = topbarobject.InputBegan:Connect(
        function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                Events[#Events+1] = input.Changed:Connect(
                    function()
    ...
    Events[#Events+1] = topbarobject.InputChanged:Connect(
        function(input)
            if
    ...
    Events[#Events+1] = UserInputService.InputChanged:Connect(
        function(input)
...
function lib:Window(text, preset, closebind)
    ...
    Events[#Events+1] = UserInputService.InputBegan:Connect(
        function(io, p)
    ...
    function lib:Notification(texttitle, textdesc, textbtn)
        ...
        Events[#Events+1] = OkayBtn.MouseEnter:Connect(
            function()
                TweenService:Create(
                    OkayBtn,
                    TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
        ...
        Events[#Events+1] = OkayBtn.MouseLeave:Connect(
            function()
                TweenService:Create(
                    OkayBtn,
        ...
        Events[#Events+1] = OkayBtn.MouseButton1Click:Connect(
            function()
                NotificationFrame:TweenSize(
                    UDim2.new(0, 0, 0, 0),
                    Enum.EasingDirection.Out,
    ...
    function tabhold:Tab(text)
        ...
        Events[#Events+1] = TabBtn.MouseButton1Click:Connect(
            function()
        ...
        function tabcontent:Button(text, callback)
            ...
            Events[#Events+1] = Button.MouseEnter:Connect(
                function()
                    TweenService:Create(
                        Button,
                        TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            ...
            Events[#Events+1] = Button.MouseLeave:Connect(
                function()
                    TweenService:Create(
                        Button,
                        TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            ...
            Events[#Events+1] = Button.MouseButton1Click:Connect(
                function()
                    pcall(callback)
                end
            )
        ...
        function tabcontent:Toggle(text,default, callback)
            ...
            Events[#Events+1] = Toggle.MouseButton1Click:Connect(
                function()
            ...
            Tab.CanvasSize = UDim2.new(0, 0, 0, TabLayout.AbsoluteContentSize.Y)
            pcall(callback, toggled)
            local hookedMetatable = { Value = toggled }
            setmetatable(hookedMetatable, {
                __index =function(tb, index)
                    local lookingfor = "value"
                    if tostring(lookingfor):lower():match(index:lower()) or tostring(index):lower()==lookingfor:lower() then 
                        return toggled
                    end;
					lookingfor = "set"
                    if tostring(lookingfor):lower():match(index:lower()) or tostring(index):lower()==lookingfor:lower() then 
                        return function(bool)
							if type(bool)~="boolean" then return "Argument #1 must be a boolean!" end
                            toggled = bool
                            if toggled then
                                TweenService:Create(
                                    Toggle,
                            ...
                            pcall(callback, toggled)
                            return bool
                            -- idk how to reverse it to set the sliders value to a custom number, if yk dm me: Linen#3485
						end
                    end;
        ...
        function tabcontent:Slider(text, min, max, start, callback)
            ...
            if type(start)=="function" then 
                ...
                callback = start;start=oldcallback
            else start = tonumber(start) or min end
            ...
            Events[#Events+1] = SlideCircle.InputBegan:Connect(
                function(input)
            ...
            Events[#Events+1] = SlideCircle.InputEnded:Connect(
                function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        dragging = false
                    end
            ...
            Events[#Events+1] = game:GetService("UserInputService").InputChanged:Connect(
                function(input)
            ...
            pcall(callback, _SliderValue)
            local hookedMetatable = { Value = _SliderValue }
            setmetatable(hookedMetatable, {
                __index =function(tb, index)
                    local lookingfor = "value"
                    if tostring(lookingfor):lower():match(index:lower()) or tostring(index):lower()==lookingfor:lower() then 
					...
					lookingfor = "set"
                    if tostring(lookingfor):lower():match(index:lower()) or tostring(index):lower()==lookingfor:lower() then 
                        return function(tbText)
							if not(tonumber(tbText)) then return "Argument #1 must be a number" end
                            -- idk how to reverse it to set the sliders value to a custom number, if yk dm me: Linen#3485
						end
                    end;
        ...
        function tabcontent:Dropdown(text, list, callback)
            ...
            callback = type(callback)=="function" and callback or function(...)return(...)end
            ...
            Events[#Events+1] = DropdownBtn.MouseButton1Click:Connect(
                function()
            ...
            for i, v in next, list do
                ...
                Events[#Events+1] = Item.MouseEnter:Connect(
                    function()
                        TweenService:Create(
                            Item,
                            TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                ...
                Events[#Events+1] = Item.MouseLeave:Connect(
                    function()
                        TweenService:Create(
                            Item,
                            TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                ...
                Events[#Events+1] = Item.MouseButton1Click:Connect(
                    function()
                        droptog = not droptog
            ...
            local hookedMetatable = { Value = selectedOption }
            setmetatable(hookedMetatable, {
                __index =function(tb, index)
        ...
        function tabcontent:Colorpicker(text, preset, callback)
            ...
            if type(callback)~="function" then callback = function(...)return (...) end end
            preset = typeof(preset)=="Color3" and preset or Color3.fromRGB(255, 255, 255)
            ...
            Events[#Events+1] = ColorpickerBtn.MouseButton1Click:Connect(
                function()
            ...
            Events[#Events+1] = Color.InputBegan:Connect(
                function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        ...
                        );Events[#Events+1] = ColorInput
                    end
                end
            )

            Events[#Events+1] = Color.InputEnded:Connect(
                function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if ColorInput then
                            ColorInput:Disconnect()
            ...
            Events[#Events+1] = Hue.InputBegan:Connect(
                function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if RainbowColorPicker then
                            return
                        ...
                        );Events[#Events+1] = HueInput
                    end
                end
            )

            Events[#Events+1] = Hue.InputEnded:Connect(
                function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if HueInput then
                            HueInput:Disconnect()
            ...
            Events[#Events+1] = RainbowToggle.MouseButton1Down:Connect(
                function()
                    ...
                    if RainbowColorPicker then
                        ...
                        while RainbowColorPicker and task.wait() and isver() do
            ...
            Events[#Events+1] = ConfirmBtn.MouseButton1Click:Connect(
                function()
                    ColorSelection.Visible = false
                    HueSelection.Visible = false
                    Colorpicker:TweenSize(
            ...
            local hookedMetatable = { Value = BoxColor.BackgroundColor3 }
            setmetatable(hookedMetatable, {
                __index =function(tb, index)
        ...
        function tabcontent:Textbox(text, disapper, callback)
            ...
            Events[#Events+1] = TextBox.FocusLost:Connect(
                function(ep)
                    if ep then
            ...
            setmetatable(hookedMetatable, {
                __index =function(tb, index)
                    ...
                    if tostring(lookingfor):lower():match(index:lower()) or tostring(index):lower()==lookingfor:lower() then 
                        return function(tbText)
							if not(type(tbText)=="string" or type(tbText)=="number") then return "Argument #1 must be a string or number" end
                            TextBox.Text = tostring(tbText)
                            pcall(callback, TextBox.Text)
						end
                    end;
        ...
        function tabcontent:Bind(text, keypreset, callback)
            local binding = false
            local Key = typeof(keypreset)=="Enum" and keypreset.Name or type(keypreset)=="string" and keypreset or nil;
            if not Key then return "You must pass a keybind! Example: tab:Bind('Bind Name', 'T', function() end)" end
            ...
            Events[#Events+1] = Bind.MouseButton1Click:Connect(
                function()
                    BindText.Text = "..."
                    binding = true
            ...
            Events[#Events+1] = game:GetService("UserInputService").InputBegan:connect(
                function(current, pressed)
                    if not pressed then
                        if current.KeyCode.Name:lower() == Key:lower() and binding == false then
                            pcall(callback)
                        end
                    end
                end
